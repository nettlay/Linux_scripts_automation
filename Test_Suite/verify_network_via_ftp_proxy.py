...
def set_ftp_proxy(**kwargs):
    case_name = kwargs.get("case_name")
    report_file = kwargs.get("report_file")
    proxy_test = kwargs.get('obj')
    rs = network_steps.set_proxy2(proxy_test, case_name, report_file)
    if rs:
        proxy_test.reboot()
    else:
        proxy_test.reset_env_halfway()
...
def access_ftp(**kwargs):
    case_name = kwargs.get("case_name")
    report_file = kwargs.get("report_file")
    proxy_test = kwargs.get('obj')
    rs = network_steps.access_ftp(proxy_test, case_name, report_file)
...
def start(case_name, kwargs):
    ...
    common_function.case_steps_run_control(steps_list, __name__, case_name=case_name,
                                           report_file=result_file, obj=ftp_test)
